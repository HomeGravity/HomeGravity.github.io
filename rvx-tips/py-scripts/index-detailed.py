...
<body>
    ...
    <div class="title">
        <h3>
            RVX 매니저 패치 JSON으로 저장 ({name}) Bata
        </h3>
    </div>
...
def StartHTML(FileName, UserName):
    patches = open_json(f"{FileName}")
    new_html = generate_html(patches)
    save_html(rf"rvx-tips\rvx-patches\rvx-patches-menu\detailed\rvx-{UserName}-detailed.html", init_html().format(name=UserName, insert_html=new_html, time=CurrentTime()))
